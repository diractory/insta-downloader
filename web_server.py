...
@app.route("/health")
def health():
    return jsonify({"status": "online", "uptime": int(time.time()-START_TIME), "bot": "Radhey #RADHEY"})
...
def web_download():
    try:
        ...
        result = subprocess.run(
            ["python3","-m","yt_dlp","-o", out_tpl,
             "--format","best[ext=mp4]/best","--no-playlist","-q", url],
            capture_output=True, text=True, timeout=120
        )

        files = [f for f in os.listdir(out_dir) if os.path.isfile(os.path.join(out_dir,f))]
        if not files:
            return jsonify({"error": "Download failed. Post may be private."}), 500
